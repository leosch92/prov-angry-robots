...
public var provEnemy : EnemyProv = null;
...
function Awake () {
	...
	{
		provEnemy = GetComponent(EnemyProv); 
	}
}

function OnDamage (amount : float, fromDirection : Vector3, source : GameObject) {
	...
	*/
	...
	if(this.gameObject.tag == "Player")
	{
		provPlayer.Prov_TakeDamage();
	}
	else if(this.gameObject.tag == "Enemy")
	{
		provEnemy.Prov_TakeDamage(source, amount);
	}
	...
	if (health <= 0)
	{
		...
		if(this.gameObject.tag == "Player")
		{
			provPlayer.Prov_Death();
		}
		else if(this.gameObject.tag == "Enemy")
		{
			provEnemy.Prov_Death(1);
		}
	}
}
	...
	if (regenerateSpeed > 0.0f) {
		while (enabled) {
			if (Time.time > lastDamageTime + 3) {
				...
				if(this.gameObject.tag == "Player")
				{
					provPlayer.Prov_Regenerate(regenerateSpeed);
				}
				else if(this.gameObject.tag == "Enemy")
				{
					provEnemy.Prov_Regenerate(regenerateSpeed);
				}
