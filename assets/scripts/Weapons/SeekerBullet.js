...
function Update () {
	...
	for (var c : Collider in hits) {
		...
		if (targetHealth) {
			// Apply damage
			targetHealth.OnDamage (damageAmount, -tr.forward, sourceSpawner);
		}
...
var sourceSpawner : GameObject;
function SetSpawner(mechSpawner : GameObject)
{
	sourceSpawner = mechSpawner;
}
