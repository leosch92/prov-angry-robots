...
public var prov : EnemyProv = null;
...
function Awake () {
	...
	// Provenance
	prov = GetComponentInParent(EnemyProv); 
}
...
function Explode () {
	...
	if (targetHealth) {
		// Provenance
			prov.Prov_Attack(damageAmount * damageFraction);
			
		// Apply damage
		targetHealth.OnDamage (damageAmount * damageFraction, character.position - player.position, this.gameObject);
	}
